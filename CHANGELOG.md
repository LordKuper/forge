...
- Added an execution plan for the desktop design-parity work
  ...
  scope — the provider permission card, per-step retry, message attachments, and the timeline's live
  status line with token rate, each with the reason it cannot ship — and in what order the rest
  arrives. One scope question the document raises is
