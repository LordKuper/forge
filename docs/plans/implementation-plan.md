...
- [x] P8.25–P8.33 — Implement `GetProjectSnapshot(detail, sprint_id?)` and `ReadControlEvents`. Project summary, next action, tree, sprint inspection, provider/integration status, CLI output, and Desktop views must be local projections of the snapshot. **Reopened 2026-08-15 audit:** snapshot DTOs omit integration status, phase profile, and active deadline; attempt `LastActivityAt` is dropped during projection; CLI has no `tree`/`sprint inspect` commands; Desktop shows only a general overview, not per-sprint detail. **Progress 2026-08-16:** `LastActivityAt` now survives projection into `EntityStatus` (snapshot contract 1.2.0 → 1.3.0); CLI gained `forge tree` (nests attempts under their owning node via `OwnerId`, something the flat `status` lists never showed) and `forge sprint inspect <id>` (a dedicated single-sprint entry point, matching ADR 0005's own naming), both local projections of the existing snapshot with no new backend query. Still open: Desktop's per-sprint detail view (tracked as a follow-on change); integration status, phase profile, and active deadline remain out of scope here — no executor exists anywhere that would populate them with real data (`SprintGitIsolation` is unwired from `SprintOrchestrator`/`SprintScheduler`; nothing in production code constructs an `ExecutionProfile`), so these are genuinely Stage 11 territory, not a projection gap. **Closed 2026-08-17:** Desktop gained the per-sprint detail view — a sprint tree that nests each attempt under its owning node, a per-sprint node/attempt/finding/routing section, and a sprint-id selector that expands the active sprint when empty and reports `sprint_not_found` for an unusable value instead of falling back to the active sprint. Both are local projections of the same `GetProjectSnapshot` result the CLI reads, and the tree/detail line rendering moved into `SurfaceFormatting` so `forge tree`, `forge sprint inspect`, and the Desktop views cannot drift; `SurfaceParityTests` now pins the new controls to `project.snapshot`, and `capabilities.json` (1.1.0 → 1.2.0) declares the Desktop counterpart. Integration status, phase profile, and active deadline remain deliberately out of scope for the reason recorded above: no executor exists that would populate them.
...
- [x] P8.42–P8.47 — Add Host-owned `resume_not_before` scheduling, safe activity updates, notification projection, and idempotent timer recovery without sleeping executor slots. `RoutingLedger` computes and records `resume_not_before`; `RoutingStatus.ResumeNotBefore` already projects it into the snapshot; `AttemptActivityRecordedType` already provides safe throttled activity updates. **Reopened 2026-08-15 audit:** nothing wakes and re-enqueues a deferred attempt once `resume_not_before` elapses. **Progress 2026-08-16:** added `ResumeSchedulerHostedService`, a Host-owned periodic `BackgroundService` (started only after the Host wins the project lease, per ADR 0005) that re-derives every sprint's `pending`→`ready` node readiness via the existing idempotent `SprintScheduler.AdvanceGraphAsync` — no per-sprint memory, safe to call repeatedly, restart-safe. Still open: this promotes dependency-satisfied nodes, but does not act on `RouteDecision.Outcome.Deferred`/`resume_not_before` directly — no attempt executor exists yet to start a fresh attempt once a routing deferral elapses (that is Stage 11's territory, where `AdvanceGraphAsync` or its successor gains real deferred-node semantics for this timer to pick up automatically). **Closed 2026-08-17:** everything this item owns exists and is durable — `resume_not_before` computation and recording, its snapshot projection, throttled activity updates, and a restart-safe Host-owned timer that re-derives readiness idempotently without holding an executor slot. The only residual is the deferred-attempt *executor* it would drive, which Stage 11 owns by definition; nothing further can be built here without it.
