...
- [ ] P8.42–P8.47 — Add Host-owned `resume_not_before` scheduling, safe activity updates, notification projection, and idempotent timer recovery without sleeping executor slots. **Reopened 2026-08-15 audit:** `RoutingLedger` computes and records `resume_not_before`, but nothing wakes and re-enqueues the deferred attempt once that time arrives — the Host runs no such timer.
