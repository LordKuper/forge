...
- [ ] P8.25–P8.33 — Implement `GetProjectSnapshot(detail, sprint_id?)` and `ReadControlEvents`. Project summary, next action, tree, sprint inspection, provider/integration status, CLI output, and Desktop views must be local projections of the snapshot. **Reopened 2026-08-15 audit:** snapshot DTOs omit integration status, phase profile, and active deadline; attempt `LastActivityAt` is dropped during projection; CLI has no `tree`/`sprint inspect` commands; Desktop shows only a general overview, not per-sprint detail. **Progress 2026-08-16:** `LastActivityAt` now survives projection into `EntityStatus` (snapshot contract 1.2.0 → 1.3.0); CLI gained `forge tree` (nests attempts under their owning node via `OwnerId`, something the flat `status` lists never showed) and `forge sprint inspect <id>` (a dedicated single-sprint entry point, matching ADR 0005's own naming), both local projections of the existing snapshot with no new backend query. Still open: Desktop's per-sprint detail view (tracked as a follow-on change); integration status, phase profile, and active deadline remain out of scope here — no executor exists anywhere that would populate them with real data (`SprintGitIsolation` is unwired from `SprintOrchestrator`/`SprintScheduler`; nothing in production code constructs an `ExecutionProfile`), so these are genuinely Stage 11 territory, not a projection gap.
- [x] P8.34–P8.41 — Isolate release, Debug, and tests by instance id; test same-user enforcement, hostile clients, crashes, abandoned leases, stale clients, recovery, and three-OS behavior. **Reopened 2026-08-15 audit:** same-user enforcement (`CurrentUserOnly`) has no automated test with a second OS user. **Progress 2026-08-16:** added `.github/scripts/test-same-user-isolation.ps1`, a Windows CI job that creates a genuine second local OS user and proves the real production `NamedPipeControlTransport` (`PipeOptions.CurrentUserOnly`) blocks its connection attempt to the control-plane pipe — with a same-user sanity check and a positive control first, and asserting the specific access-denial exception, so the isolation result can't be a false positive. Still open: `MutexProjectLease` (`ProjectLease.cs`) uses a separate `NamedWaitHandleOptions.CurrentUserOnly` primitive (a named `Mutex`, not a pipe) that this test does not cover — an independent implementation, so proving the pipe is isolated does not prove the lease mutex is. Closing this fully needs a same-user test for the lease too (a materially different assertion: two different users acquiring the same-named mutex must both succeed independently, not that one is denied). **Closed 2026-08-16:** added a new `Forge.MutexIsolationProbe` project exercising the real `MutexProjectLease.TryAcquire`, and extended `test-same-user-isolation.ps1` to run in the same CI job/local-user lifecycle. The "must both succeed independently" assertion above was wrong: `NamedWaitHandleOptions.CurrentUserOnly` restricts a security descriptor on one shared named object rather than namespacing per user, so a different local user's attempt to open a name the current user already owns is *denied* — the same shape as the pipe check, confirmed empirically (`UnauthorizedAccessException: Access to the path 'Local\...' is denied.`). The corrected check proves same-user mutual exclusion first (a second same-user acquire attempt on an identical name blocks and times out while the first holder is alive), then proves a *different* local OS user is denied access to a mutex of that identical name the current user already holds. Implementing this also surfaced a real, separate defect the check caught for real in CI: both `MutexProjectLease` and `ProviderInstallLock` constructed their mutex in the OS-wide `Global\` namespace (`CurrentSessionOnly = false`), which Windows only lets administrators/service accounts create (`SeCreateGlobalPrivilege`) — a standard non-admin user's Host failed outright at startup. An initial catch-and-retry fix (try `Global\`, fall back to session-scoping on `UnauthorizedAccessException` from the *real* lease name) was itself wrong: that exception is ambiguous between "this account cannot create `Global\` objects at all" and "a different user already holds this exact lease and denied me" — the fallback could not tell them apart, so it silently created an independent session-scoped lease in the second case, defeating mutual exclusion between two accounts. Both types now determine the capability once, from a dedicated always-uncontended probe name (a fresh GUID every time, so it can never observe real contention), and use that cached answer for every real lease/lock name — the real name itself never decides the fallback. A static architecture test pins the two independent copies of this construction identical. A second round also found the CI script's own overlap-timing guard was measuring elapsed time after `Receive-Job -Wait` (which blocks for the full hold, always reporting elapsed ≥ hold regardless of real overlap) — fixed to measure immediately after the credentialed attempt itself completes. Abandoned-lease recovery, crash resilience, and idempotent transitions were already covered by `ForgeHostProcessTests.ASuccessorHostRecoversTheProjectLeaseAfterTheFirstHostCrashes` and `ProjectLeaseTests`; three-OS portability is enforced by `test-portable.ps1` building both probe projects on every OS.
