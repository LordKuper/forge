...
- [x] P8.42–P8.47 — Add Host-owned `resume_not_before` scheduling, safe activity updates, notification projection, and idempotent timer recovery without sleeping executor slots. `RoutingLedger` computes and records `resume_not_before`; `RoutingStatus.ResumeNotBefore` already projects it into the snapshot; `AttemptActivityRecordedType` already provides safe throttled activity updates. **Closed 2026-08-16:** `ResumeSchedulerHostedService` now runs in the Host as a periodic, idempotent `BackgroundService` that re-derives every sprint's readiness via `SprintScheduler.AdvanceGraphAsync` — no per-sprint memory, safe to call repeatedly, restart-safe. It re-enqueues sprint progress once a deferral elapses without any live timer to lose; no attempt executor exists yet to consume `RouteDecision.Outcome.Deferred` directly (Stage 11), so this is scoped to what the Host itself owns today.
