...
| Stage 7 independent review, round 2 (PR #21) | Independent review at `https://github.com/LordKuper/forge/pull/21#pullrequestreview-4897672509` re-verified round 1's fixes (F1/F2 genuinely fixed; F3/F4 only partly) and found 3 new blocking issues, all fixed: CI had never actually been green on this PR (`Validate .NET solution` failed on both prior commits — 6 and 7 of 316 tests — which round 1 did not catch since it reviewed the diff rather than checking CI; a real Windows CI runner reproducibly failed `git worktree add` for an *attempt* worktree with `Win32Exception: The directory name is invalid`, .NET's own message for a working directory that does not exist, at a full-GUID-length nested path `core.longpaths` alone did not save on that machine) — every id in `WorktreeLayout` (`Forge.Application.SprintGitIsolation.cs`) is now a 16-hex-character (64-bit) prefix instead of a full GUID, cutting worktree path depth well below any observed failure point, and `GitTestRepository`/`ProcessRunner`-style calls in test code no longer crash with an unhandled `Win32Exception` on a missing working directory; round 1's own claimed self-heal for a deleted worktree directory was verified false — `git worktree prune` clears a stale path registration but never touches the branch, so a bare retry failed forever with "branch already exists" (or would have silently discarded real integrated history had it force-deleted the branch instead) — `GitWorktreeManager.CreateAsync` now checks whether the branch still exists after a failed create-and-prune and re-attaches a new worktree to that *existing* branch instead, never force-deleting one; and `FileRoutingStore.GetRouteDecisionsAsync`'s new truncate-on-torn-line write (added responding to round 1's F1) took no lock, reintroducing exactly the class of race F1 existed to close, this time on the read path — it now holds the same per-path lock `AppendRouteDecisionAsync` does. `dotnet restore Forge.slnx --locked-mode`; `dotnet format Forge.slnx --no-restore --verify-no-changes`; `dotnet build Forge.slnx --no-restore --configuration Release`; `dotnet test Forge.slnx --no-build --configuration Release` (317 tests, +1: recovering a deleted integration-worktree directory preserves its already-integrated commit rather than reverting to the sprint's original frozen base; 2 existing tests strengthened to assert `GitOperationResult.CleanupSucceeded`/`DiscardAttemptAsync`'s own boolean return on the clean path, which had zero coverage). Pushed as commit to be confirmed green on the real CI runner before the review gate closes. | 2026-08-10 |
| Stage 7 independent review, round 2 response, remaining findings (PR #21) | Beyond round 2's 3 blocking issues (previous row), the same review also posted 9 non-blocking findings, all addressed: genuine (non-torn) route-decision corruption now surfaces uniformly as `InvalidDataException` instead of whichever raw exception type each field parser happened to throw, matching `FileSprintEventLog.AppendTransitionAsync`'s own precedent for this exact distinction; `RoutingLedger.RecordOutcomeAsync` now takes the exact `RouteDecision` its caller's `DecideAsync` call returned (not just node/attempt/key) and refunds the retry budget only when that decision was actually `Routed`, so reporting an outcome against a `CircuitOpen`/`BudgetExhausted` decision can no longer credit a unit that was never spent; every budget/breaker read-modify-write in `RoutingLedger` is now serialized per sprint by one in-process lock shared by `DecideAsync`/`RecordOutcomeAsync`, closing a lost-update race between concurrently routed nodes and making the ADR's single-process-lock-parity claim actually true; `GitWorktreeManager.RemoveAsync` no longer reports a leaked, no-longer-registered directory as a clean removal — it now deletes any directory still physically present after pruning, regardless of whether `git` itself still had it registered; every `git` failure path in `GitWorktreeManager` now carries the failing invocation's own (truncated) `stderr` through a new `GitOperationResult.Detail`, so a future CI failure is diagnosable from the log instead of only a fixed diagnostic code; `GitTestRepository` now isolates every git invocation from the host's global/system config (`GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM`/`GIT_CONFIG_NOSYSTEM`, plus an explicit `commit.gpgsign=false`) and checks `git init`'s own exit code, and `HeadAsync` now throws on a failed `rev-parse` instead of silently returning an empty string; `SprintGitIsolation.IntegrateAsync`/`RebaseAttemptAsync`/`CreateAttemptWorktreeAsync` no longer let `GetHeadAsync`'s `InvalidOperationException` escape past their own `GitOperationResult` contract — a new `TryGetHeadAsync` converts it into `worktree_unavailable`; and the interface doc for `IWorktreeManager.ExistsAsync` and ADR 0004 were updated to describe its actual (registered-and-present) behavior and `GitOperationResult.CleanupSucceeded`, neither of which the prior text mentioned. `ProcessRequest` gained an optional `EnvironmentVariables` map (additive, defaults to inherited, used only by the isolated test harness) to make the git-config isolation possible. `dotnet restore Forge.slnx --locked-mode`; `dotnet format Forge.slnx --no-restore --verify-no-changes`; `dotnet build Forge.slnx --no-restore --configuration Release`; `dotnet test Forge.slnx --no-build --configuration Release` (319 tests, +2: reporting an excluded failure against a decision that was never actually routed does not credit a phantom budget unit, and genuine route-decision-log corruption surfaces as `InvalidDataException`; the discard and reconcile tests were strengthened to assert the worktree directory and its git registration are both actually gone, not merely inferred from the now-stricter `ExistsAsync`). | 2026-08-10 |
| Stage 7 independent review, round 3 (final; PR #21) | Independent review at `https://github.com/LordKuper/forge/pull/21#pullrequestreview-4898045340` — the third and last full-scope round this repo's policy allows — first independently re-verified CI was genuinely green on the current head and re-derived all 12 prior findings against the actual code (all confirmed genuinely fixed, none superficial; explicitly checked and cleared a novel short-id/self-heal wrong-branch-attach concern and `ReconcileAsync`'s short-id matching as sound), then found 4 new non-blocking issues from reviewing the complete picture across all commits rather than one incremental diff, all fixed: `RecordOutcomeAsync`'s `Routed`-only gate from the prior round covered the budget refund but not the circuit breaker update below it, so reporting a success against a `CircuitOpen`/`BudgetExhausted` decision (nothing was ever actually attempted) could still close a breaker nothing tested, or reporting a failure could reopen it with a fresh cooldown on every misreport — a livelock that would never let the breaker recover; fixed by moving the `decision.Outcome != Routed` check to the top of the method as a full no-op, covering both the budget and breaker paths uniformly. `RecoverIfDirtyAsync` was the one caller of `GetHeadAsync` that the prior round's `TryGetHeadAsync` conversion missed, so `EnsureIntegrationWorktreeAsync` could still throw past its own `GitOperationResult` contract (deterministically, not only via a TOCTOU race — e.g. deleting the `.git` file inside an otherwise-present worktree directory); now routed through `TryGetHeadAsync` like every other call site. `GitOperationResult.Detail` (added the previous round for CI diagnosability) had zero readers anywhere in `src/` or `tests/`, so the diagnosability goal was not actually delivered despite being claimed — every worktree-related test assertion now goes through a shared `AssertSucceeded` helper that includes `Detail` in its failure message. `IWorktreeManager.CreateAsync`'s own doc comment still described the pre-self-heal behavior ("never reuses or resets an existing" branch, always returns the requested commit) after the branch-preserving self-heal made both claims false; corrected. Also corrected two nits raised in the review's summary (not opened as separate threads): `WorktreeLayout.ShortId`'s doc claimed "64 bits" where a v4 GUID's fixed version nibble falling inside that 16-hex-character prefix actually leaves 60 bits of randomness. `dotnet restore Forge.slnx --locked-mode`; `dotnet format Forge.slnx --no-restore --verify-no-changes`; `dotnet build Forge.slnx --no-restore --configuration Release`; `dotnet test Forge.slnx --no-build --configuration Release` (320 tests, +1: reporting either a success or a failure against a decision the breaker itself already refused leaves the breaker completely untouched in both directions). No further full-scope independent review round follows per policy; the gate closes once these findings' threads are resolved. | 2026-08-10 |
