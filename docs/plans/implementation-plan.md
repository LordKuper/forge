...
- [x] P8.34–P8.41 — Isolate release, Debug, and tests by instance id; test same-user enforcement, hostile clients, crashes, abandoned leases, stale clients, recovery, and three-OS behavior. **Reopened 2026-08-15 audit:** same-user enforcement (`CurrentUserOnly`) has no automated test with a second OS user. **Closed 2026-08-16:** added `.github/scripts/test-same-user-isolation.ps1`, a Windows CI job that creates a genuine second local OS user and proves the real production `NamedPipeControlTransport` (`PipeOptions.CurrentUserOnly`) blocks its connection attempt to the control-plane pipe, with a same-user sanity check first so the isolation result can't be a false positive from a broken probe.
