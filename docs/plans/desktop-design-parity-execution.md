...
More landed than the prompt listed: #114 (ADR 0057 sprint title), #115 (ADR 0058 gate as linked action), #116 (ADR 0059 diff payload envelope), #117 (ADR 0060 tool-call capture), #118 (ADR 0061 usage capture), #119/#120 (ADR 0062/0063 model+effort routing), #121 (ADR 0064 payload rendering), #122 (ADR 0065 sidebar rows).
...
| A4 | Not started | No `KeyboardAccelerator` in `src/Forge.Desktop` (grep clean). |
...
| C3 | Not started | No popovers. Stage graph is buildable from `currentDetails.Nodes`. No GitHub code anywhere in `src`. Sprint plan has no backing data. |
...
| # | Status | Evidence |
|---|---|---|
| D1 | **Partially done** (PR #121 / ADR 0064) | **Done:** payload chips via `TimelineCardProjector.BuildStats` (`SprintWorkspace.cs:632-642`, `StatChip` `:434-474`), lazy detail rows `:655-692`, per-type glyphs `:409-419`, operator bubble / agent sparkle / system card `:721-764`. **Remains:** primary line is still one mono string `* <ISO> [type/actor] <msg>` `:618-626`; no per-tool-call card with duration; no inline diff hunks (hunks never persisted, ADR 0059); no test output/exit code (never captured, ADR 0006/0060); no error card + "Retry step" (no per-tool-call failure/retry model); **no permission card — ruled out by ADR 0058** (needs an interactive provider protocol Forge does not have); no live status line/token rate. |
...
Each slice is one branch / one PR / one ADR, matching how B1+B2 shipped as ADR 0065.

**Wave 0 — contract & backend (unblocks the UI waves)**
1. **S1 · Sprint timing and worktree/branch in the header contract.** Adds a durable sprint start timestamp and surfaces the derivable integration-branch/worktree path through `SprintWorkspaceSummary`/`SprintStatusHeaderData`. *Blocked on Q8.* Blocks S13.
2. **S2 · Sprint-level diff statistics.** On-demand `DiffStatAsync` of the integration branch vs `BaseSha`, exposed on the workspace summary. *Blocked on Q9.* Blocks S13.
3. **S3 · Provider model enumeration and per-sprint model selection.** `ILlmProvider.ListModelsAsync`, `CreateSprintCommand.Model`, `ModelPolicyGate` validation of the chosen value. *Blocked on Q10, Q11.* Blocks S15.
4. **S4 · Settings configuration schema.** New keys for approval mode, theme, provider priority, per-model effort; `configuration.json` + `user-config.schema.json` + resolver. *Blocked on Q22, Q23, Q24(shape only).* Blocks S16, S17.
5. **S5 · Provider quota posture.** Per Q5(a), no new signal: formalize "no limit data" as a first-class rendering state. *Blocked on Q5.* Blocks S10.
6. **S6 · Attempt token rollup for the composer.** Per Q16(b), expose the latest attempt's total on the workspace summary. *Blocked on Q16.* Blocks S23.

**Wave 1 — structural view work**
7. **S7 · Page chrome: remove the global header, give every page its own.** A1, F1, G5, C6's heading dedup. *Blocked on Q2.* Blocks S8, S12, S18, S19.
8. **S8 · Dissolve `ContextualActionHost`.** A2, D2's remaining half, Stop relocation. *Blocked on Q3.* Depends on S7.
9. **S9 · Sidebar batch 2.** B3, B4, B5, B6. *Blocked on Q4.* Independent of S7.
10. **S10 · Sidebar footer: pinned, chipped.** A3, B7 rendering. *Depends on S5.*

**Wave 2 — timeline and composer**
11. **S11 · Timeline presentation pass.** D4, D5, D6, C5. *Blocked on Q13.* Independent.
12. **S12 · Pinned multi-line composer.** E1 remainder, E2 relocation. *Blocked on Q15, Q19.* Depends on S7, S8.
13. **S13 · Sprint header stats and progress bar.** C1 rendering, C4. *Depends on S1, S2.*
14. **S14 · Header popovers.** C3 — stage graph + working diff only. *Blocked on Q12.* Depends on S13.
15. **S15 · Model picker UI.** C2, F6. *Blocked on Q10, Q11.* Depends on S3.
16. **S16 · Timeline failure card.** D1's error-card half per Q7(a). *Blocked on Q7.* Depends on S8, S11.

**Wave 3 — settings pages**
17. **S17 · Project settings redesign.** F2, F3, F4, F5. *Blocked on Q18, Q19.* Depends on S7.
18. **S18 · Forge settings redesign.** G1, G2, G3 UI, section rail. *Blocked on Q20, Q21, Q22.* Depends on S4, S7.
19. **S19 · Project overview restyle.** H1. *Blocked on Q25.* Depends on S7.

**Wave 4 — visual system and polish**
20. **S20 · Token sheet completion.** I1 radius/spacing tokenization with the live-run verification `App.xaml` defers, plus I4's glyph additions. Blocks S21 (needs `Spinner`).
21. **S21 · Motion and hover.** I2, I3, and D3's spinner + "last updated" line. *Blocked on Q14.* Depends on S20.
22. **S22 · Keyboard accelerators and the rail glyph.** A4, A5. *Blocked on Q1.* Depends on S20.
23. **S23 · Composer context counter.** E3 per Q16(b); the attach chip is dropped per Q17. *Depends on S6, S12.*
24. **S24 · Light theme.** G4. *Blocked on Q24 and on an external design deliverable.* Depends on S18, S20. Schedule last.

**Critical path:** Q8/Q9 → S1+S2 → S13 → S14; and Q3 → S7 → S8 → S12. Everything in Wave 0 can start in parallel once Q5, Q8, Q9, Q10, Q11, Q16, Q22, Q23 are answered. S9, S11, S20 are answerable-and-startable immediately after Q1, Q4, Q13.

## Part 4: Decisions (recorded 2026-08-28)

User accepted every recommended default in Part 2 as-is. Binding answers for implementers:
...
| Q14 (D3 live updates) | Keep 15s poll, add spinner + "last updated" line. |
...
| Q22 (G1 approval mode) | Map to existing `interaction.confirm_destructive` + a new auto-approve-gate setting; keep the mandatory-gate disclaimer. |
...
Execution proceeds through the waves in Part 3, one branch/PR/ADR per slice, full AGENTS.md review loop per PR (≤3 full-scope + critical-only thereafter), sequenced to respect the dependency graph. S1+S2 are combined into one slice (`Sprint header: elapsed time and diff statistics`) since both are additive fields on the same read model feeding S13, mirroring how B1+B2 were combined for the same reason.
