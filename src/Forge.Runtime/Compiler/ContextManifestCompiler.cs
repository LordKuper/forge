...
public static class ContextManifestCompiler
{
        ...
        int tokenBudget)
    {
        ...
        int allocated = tokenBudget - remaining;
        ...
        return new(
            ...
            tokenBudget, allocated, layers, truncated, digest);
    }

    /// <summary>Attaches a <see cref="ContextResultBundle"/> (ADR 0012's layer 4) to an already
    /// compiled manifest, recomputing <see cref="ContextManifest.ManifestDigest"/> and
    /// <see cref="ContextManifest.AllocatedTokens"/> so both still reflect the manifest's full
    /// content. A query result's estimated token cost is its content's chars/4 estimate — the same
    /// heuristic <c>ForgeDocumentCompiler</c> uses, so every layer counts tokens the same way.</summary>
    public static ContextManifest WithQueryResults(ContextManifest manifest, ContextResultBundle bundle)
    {
        ArgumentNullException.ThrowIfNull(manifest);
        ArgumentNullException.ThrowIfNull(bundle);

        List<ContextManifestItem> queryResults = [.. bundle.Results
            .Where(result => result.Diagnostic == ContextQueryOperationDiagnostic.None && result.ContentDigest is not null)
            .OrderBy(result => result.OperationId, StringComparer.Ordinal)
            .Select(result => new ContextManifestItem(
                result.OperationId,
                result.ContentDigest!,
                EstimateTokens(result.ByteCount),
                "query_result"))];

        ContextManifestLayers layers = manifest.Layers with { QueryResults = queryResults };
        int allocated = manifest.Layers.Rules.Sum(item => item.EstimatedTokens) +
            manifest.Layers.Knowledge.Sum(item => item.EstimatedTokens) +
            queryResults.Sum(item => item.EstimatedTokens);
        ...
        return manifest with { Layers = layers, AllocatedTokens = allocated, ManifestDigest = digest };
    }

    private static List<ContextManifestItem> Admit(
        IEnumerable<ForgeDocument> ordered,
        string rationale,
        List<ContextManifestTruncatedItem> truncated,
        ref int remaining)
    {
        List<ContextManifestItem> admitted = [];
        foreach (ForgeDocument document in ordered)
        {
            if (document.EstimatedTokens <= remaining)
            {
                admitted.Add(new(document.RelativePath, Digest(document.Body), document.EstimatedTokens, rationale));
                remaining -= document.EstimatedTokens;
            }
            else
            {
                truncated.Add(new(document.RelativePath, document.EstimatedTokens, TruncatedReason));
            }
        }
    ...
    private static int EstimateTokens(int byteCount) => (byteCount + 3) / 4;
